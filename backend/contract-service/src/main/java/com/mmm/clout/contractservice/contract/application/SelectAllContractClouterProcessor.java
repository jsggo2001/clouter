...
public class SelectAllContractClouterProcessor {
    ...
    public Page<ContractReader> execute(Long id, Pageable pageable) {
        ...
        List<Contract> contracts = contractRepository.findAllByClouterInfoClouterId(id, pageable);
        List<ContractReader> result = contracts.stream().map(contract -> new ContractReader(contract)).collect(Collectors.toList());

        JPAQuery<Contract> countQuery = contractRepository.findAllByClouterInfoClouterIdForCount(id);
